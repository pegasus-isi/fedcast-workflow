...
echo "== Phase C: training (2 units, capped sequences — CPU-feasible) =="
for MODE in centralized federated; do
    M=$([ "$MODE" = centralized ] && echo cen || echo fed)
    # shellcheck disable=SC2086
    $PY_TRAIN "$BIN/train_dgmr.py" --mode "$MODE" $CLIENTS \
        --interval-months 1 --archive-start "$MONTH" --archive-months 1 \
        --segment-index 0 --segment-size 2 --total-units 2 \
        --validate-every 1 --seed 42 --aggregation uniform \
        --limit-train-sequences 4 \
        --state-out "${M}_L1_seg0_state.tar.gz" \
        --best-out "${M}_L1_best.ckpt"
done
